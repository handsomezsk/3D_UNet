...
from model.Unet3D import UNet3D  # 还有一些问题，暂时先不用这个模型
...
def train(model, train_loader, optimizer, loss_func, alpha):
...
if __name__ == '__main__':
    args = config.args
    #save_path = os.path.join(args.save, 'UNet')
    ...
    for epoch in range(start_epoch, args.epochs + start_epoch ):
        ...
        train_log = train(model, train_loader, optimizer, loss, alpha)
