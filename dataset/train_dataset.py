...
from .transforms import Compose, Window, MinMaxNorm, RandomRotate


class TrainDataset(Dataset):

    def __init__(self, args, image_dir, label_dir, num_samples=4, train=True):
        self.args = args
    ...
    def __getitem__(self, idx):
        ...
        image_arr = sitk.GetArrayFromImage(image).astype(np.float)
        ...
        if self.transforms:
            image_rois = [self.transforms(image_roi)
                for image_roi in image_rois]

        image_rois = torch.tensor(np.stack(image_rois)[:, np.newaxis],
            dtype=torch.float)
        label_rois = torch.tensor((np.stack(label_rois) > 0).astype(np.float)[:, np.newaxis],
            dtype=torch.float)
    ...
    @staticmethod
    def get_dataloader(args, dataset, shuffle=False):
        return DataLoader(dataset, args.batch_size, shuffle,
            num_workers=args.n_threads, collate_fn=TrainDataset.collate_fn)
